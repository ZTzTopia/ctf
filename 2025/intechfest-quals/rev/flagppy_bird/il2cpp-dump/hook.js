...
var SIZE_T = PTR_SIZE === 8 ? "ulong" : "uint";
var MASK64 = ptr("0x00ffffffffffffff");
var untag = (p) => Process.arch === "arm64" ? p.and(MASK64) : p;
function tryReadUtf8String(p) {
  if (p.isNull()) {
    return "";
  }
  try {
...
function looksPrintableId(s) {
  return s.length > 0 && s.length < 256 && /^[\w.`$<>+,-]+$/.test(s);
}
function readMethodNameViaStruct(mi) {
  const base = untag(mi);
  for (let off = 0; off < 128; off += PTR_SIZE) {
  ...
  setTimeout(() => {
    frida_java_bridge_default.enumerateLoadedClasses({
      ...
      onComplete: () => {
        ...
        const il2cpp_thread_attach = new NativeFunction(m.findExportByName("il2cpp_thread_attach"), "pointer", ["pointer"]);
        const il2cpp_domain_get = new NativeFunction(m.findExportByName("il2cpp_domain_get"), "pointer", []);
        const il2cpp_domain_get_assemblies = new NativeFunction(m.findExportByName("il2cpp_domain_get_assemblies"), "pointer", ["pointer", "pointer"]);
        const il2cpp_assembly_get_image = new NativeFunction(m.findExportByName("il2cpp_assembly_get_image"), "pointer", ["pointer"]);
        const il2cpp_image_get_name = new NativeFunction(m.findExportByName("il2cpp_image_get_name"), "pointer", ["pointer"]);
        const il2cpp_image_get_class_count = new NativeFunction(m.findExportByName("il2cpp_image_get_class_count"), SIZE_T, ["pointer"]);
        const il2cpp_image_get_class = new NativeFunction(m.findExportByName("il2cpp_image_get_class"), "pointer", ["pointer", SIZE_T]);
        const il2cpp_class_get_type = new NativeFunction(m.findExportByName("il2cpp_class_get_type"), "pointer", ["pointer"]);
        const il2cpp_type_get_name = new NativeFunction(m.findExportByName("il2cpp_type_get_name"), "pointer", ["pointer"]);
        const il2cpp_class_get_methods = new NativeFunction(m.findExportByName("il2cpp_class_get_methods"), "pointer", ["pointer", "pointer"]);
        const il2cpp_method_get_name = new NativeFunction(m.findExportByName("il2cpp_method_get_name"), "pointer", ["pointer"]);
        const il2cpp_method_get_param_count = new NativeFunction(m.findExportByName("il2cpp_method_get_param_count"), "uint", ["pointer"]);
        setTimeout(() => {
          try {
         ...
         assemblyName: ${asmName}`);
              ...
              for (let j = 0; j < classCount; j++) {
                ...
                const pName2 = untag(klass).add(2 * PTR_SIZE).readPointer();
                const name2 = tryReadUtf8String(pName2);
                console.log(`             [*] Class ${j}: ${klass} - ${name2}`);
